...
void dump_section_content(Binary *bin, std::string &secname)
{
  size_t i;
  Section sec;
  ...
  {
    sec = bin->sections[i];
    if (!secname.compare(sec.name))
			break;
  }
  if (i == bin->sections.size())
  {
    fprintf(stderr, "file ´%s´ does not have a ´%s´ section \n", bin->filename.c_str(),
            secname.c_str());
    return;
  }
  printf("\n´%s´ section:", secname.c_str());
  for (i = 0; i < sec.size; i+= 2)
  {
    uint8_t byte1 = sec.bytes[i];
    uint8_t byte2 = 0;
    if (i + 1 < sec.size)
    {
      byte2 = sec.bytes[i + 1];
